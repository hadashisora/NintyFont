...
// Parts based on BRFNTify-Next copyright 2009-2014 Tempus, RoadrunnerWMC
...
// This is the source file for BinaryRevolutionFont
// font format class
#include <algorithm>
#include <vector>
#include <QDebug>
...
#include "binaryrevolutionfont.h"
...
namespace NintyFont::RVL
{
    ...
    BinaryRevolutionFont::BinaryRevolutionFont(std::string filePath)
        : FontBase()
    {
        //Various finds:
        //
        ...
        Format::RFNT *rfnt = nullptr;
        NTR::Format::FINF *finf = nullptr;
        Format::TGLP *tglp = nullptr;
        ...
        try
        {
            ...
            //Parsing metadata
            //Read RFNT header
            rfnt = new Format::RFNT(br);
            rfnt->validateSignature(); //This function call is essentially an internal check for both the file being correct and for code working correctly
            if (rfnt->version != 0x104) throw std::runtime_error(fmt::format("Unknown format version 0x{:X}", rfnt->version));
            //Read FINF header
            br->setPosition(rfnt->ptrInfo);
            ...
            for (uint16_t i = 0; i < tglp->sheetCount; i++)
            {
                ...
                //sheet->save(QString::fromStdString(fmt::format("dec_sheet_{}.png", i)), "PNG");
            }
            ...
            switch(finf->encoding)
            {
                ...
                case (uint8_t)CharEncodings::CP1252:
                    ...
                    break;
                default:
                    throw std::runtime_error(fmt::format("NintyFont runtime error in {} at line {}: Unknown BRFNT glyph encoding!", __FILE__, __LINE__));
            }
            ...
            fontProperties = new std::vector<PropertyList::PropertyBase *>
            {
                ...
                new PropertyList::Property<uint16_t>(fontPropDescriptors->at(NITROFONT_PROP_VERSION), rfnt->version),
    ...
    void BinaryRevolutionFont::saveBinaryFont(std::string filePath)
    {
        ...
        for (uint32_t i = 0; i < sheetImgs.size(); i++)
        {
            ...
            sheets[i] = Image::TextureCodec::encodeTexture(Image::TextureCodec::returnPlatformTextureType(Format::getRvlFontImageFormat(fontProperties)), sheetImgs[i]->bits(), sheetSize.first, sheetSize.second);
        }
    ...
    void BinaryRevolutionFont::recalculateSheetInfo(int32_t pixels)
    {
    ...
    std::vector<PropertyList::PropertyListEntryDescriptor *> *BinaryRevolutionFont::getGlyphPropertyDescriptors()
    {
        return glyphPropDescriptors;
    }

    std::vector<PropertyList::PropertyBase *> *BinaryRevolutionFont::getPropertiesList()
    {
        return fontProperties;
    }

    std::pair<uint32_t, uint32_t> BinaryRevolutionFont::getGlyphImageSize()
    {
        return cellSize;
    }

    bool BinaryRevolutionFont::canCreateCopyGlyphs()
    {
        return true;
    }

    bool BinaryRevolutionFont::canDeleteGlyphs()
    {
        return true;
    }

    Glyph *BinaryRevolutionFont::createEmptyGlyph()
    {
        std::vector<PropertyList::PropertyBase *> *props = new std::vector<PropertyList::PropertyBase *>
        {
            new PropertyList::Property<uint16_t>((*glyphPropDescriptors)[NITROGLYPH_PROP_INDEX], 0U),
    ...
    std::vector<PropertyList::PropertyBase *> *BinaryRevolutionFont::getDrawablePropertiesList()
    {
        return drawableProperties;
    }

    void BinaryRevolutionFont::drawDrawableProperties(QPainter *painter, uint32_t rows, uint32_t columns)
    {
        //Linefeed
    ...
    bool BinaryRevolutionFont::identifyFile(uint8_t *bytes)
    {
        ...
        uint8_t magicLE[] = {0x54, 0x4E, 0x46, 0x52};
        return BinaryTools::BinaryReader::bytearraycmp(magicBE, bytes, 4) || BinaryTools::BinaryReader::bytearraycmp(magicLE, bytes, 4);
    }

    std::string BinaryRevolutionFont::returnFileTypeString()
    {
        return "NW4R font files (*.brfnt *.rfnt)";
    }

    std::string BinaryRevolutionFont::returnFileExtensionString()
    {
        return "*.brfnt *.rfnt";
    }

    BinaryRevolutionFont::~BinaryRevolutionFont()
    {
